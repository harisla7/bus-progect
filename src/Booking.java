...
//		button2.setEnabled(true);
    ...
    private void setCbx() {
        try {
            ResultSet rst = DBConnection.getDBConnection().createStatement(
                    ...
                    ResultSet.CONCUR_UPDATABLE).executeQuery("SELECT bus_no,departure_time From Schedules where route_name='" + combo7.getSelectedItem() + "'");
    ...
    private void amount() {
        try {
            ResultSet rst = DBConnection.getDBConnection().createStatement(
                    ResultSet.TYPE_SCROLL_INSENSITIVE,
                    ResultSet.CONCUR_UPDATABLE).executeQuery("SELECT fare_charged FROM routes where route_name='" + combo7.getSelectedItem() + "'");
    ...
    private void passroute() {
        try {
            ResultSet rst = DBConnection.getDBConnection().createStatement(
                    ResultSet.TYPE_SCROLL_INSENSITIVE,
                    ResultSet.CONCUR_UPDATABLE).executeQuery("SELECT route_name FROM schedule");
    ...
    private void setroute() {
        try {
            ResultSet rst = DBConnection.getDBConnection().createStatement(
                    ResultSet.TYPE_SCROLL_INSENSITIVE,
                    ResultSet.CONCUR_UPDATABLE).executeQuery("SELECT departure,arrival FROM routes");
    ...
    private void book() {
        String SQL;
        SQL = "SELECT * FROM booking WHERE seano='" + combo6.getSelectedItem() + "'  AND bus_no='" + combo1.getSelectedItem() + "'AND date_of_travel='" + t_date.getText() + "'";
        ...
        try {
            ...
            if (recordfound) {
                ...
                try {
                    Statement statement = DBConnection.getDBConnection().createStatement();
                    {
                        String temp = "INSERT INTO booking(bookingno,pass_no,pass_name,bus_no,seatno,date_of_travel,time_of_travel,departure,arrival,amount) VALUES ('" +
                                text1.getText() + "', '" +
    ...
    private void generator() {
        ...
        try {
            ResultSet rst = DBConnection.getDBConnection().createStatement(
                    ResultSet.TYPE_SCROLL_INSENSITIVE,
                    ResultSet.CONCUR_UPDATABLE).executeQuery("SELECT bookingno FROM booking");
